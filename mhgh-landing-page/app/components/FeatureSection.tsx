			   ...
			   <section className="relative w-full flex flex-col items-center justify-center py-10 md:py-20">
					   ...
					   <div className="w-full flex flex-col items-center z-10 mb-6">
							   <div className="hidden md:flex justify-center mb-6">
									   <Image src="/featureLabel.webp" alt="Feature Label Desktop" width={400} height={100} className="w-auto h-20 md:h-22 xl:h-24" priority />
							   </div>
							   <div className="flex md:hidden justify-center mb-4">
									   <Image src="/featureMobileLabel.webp" alt="Feature Label Mobile" width={200} height={50} className="w-auto h-12" priority />
							   </div>
					   </div>
					   {/* Custom 3D Carousel */}
					   <div className="relative w-full max-w-5xl mx-auto">
							   ...
							   <div className="absolute md:-left-10 left-2 top-1/2 -translate-y-1/2 z-30 cursor-pointer">
								   <button onClick={() => setActiveIdx((activeIdx - 1 + features.length) % features.length)} className="bg-transparent border-none">
									   <Image src="/arrow.png" alt="Trái" width={40} height={40} className="cursor-pointer md:w-[40px] md:h-[40px] w-[22px] h-[22px]"/>
								   </button>
							   </div>
							   <div className="absolute md:-right-10 right-2 top-1/2 -translate-y-1/2 z-30 cursor-pointer">
								   <button onClick={() => setActiveIdx((activeIdx + 1) % features.length)} className="bg-transparent border-none rotate-180">
									   <Image src="/arrow.png" alt="Phải" width={40} height={40} className="cursor-pointer md:w-[40px] md:h-[40px] w-[22px] h-[22px]"/>
								   </button>
							   </div>
					   </div>
			   </section>
